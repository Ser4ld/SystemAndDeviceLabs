...
double compute_calc(const std::vector<double>& v1, const std::vector<double>& row){
    double sum = 0.0;
...
int main(int argc, char **argv){
    ...
    for (int i=0; i<n; i++){
        futures.push_back(std::async(std::launch::async, compute_calc, ref(v1), ref(mat[i])));
    }

    std::vector<double> v(n);
    
    for (int i=0; i<n; i++){
        v[i] = futures[i].get(); // get the value from the future
    }
